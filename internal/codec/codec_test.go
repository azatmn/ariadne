...
// Круговой прогон: закодировали, раскодировали, получили то же самое.
//
// Время намеренно взято со СМЕЩЕНИЕМ +03:00, а не в UTC. Backend присылает
// местное время с зоной, и сравнение идёт через Time.Equal — сравнением
// структур такой же момент в другой зоне считался бы расхождением, а нам
// важен момент, а не запись.
//
// Координаты сравниваются точно, без допуска: в них пять знаков после
// запятой, это около метра, и округление на входе сдвинуло бы снэпы.
...
// Первый слой обёртки — base64. Вход приходит снаружи и по определению
// враждебен: до распаковки и разбора JSON тут дело не доходит вовсе.
...
// Бомба сжатия: маленький вход, огромный выход.
//
// Тест сначала убеждается, что данные жмутся больше чем в десять раз, — иначе
// он проверял бы не потолок, а собственную выдумку. Потом ставит лимит ниже
// распакованного размера и требует именно ErrDecompressedTooLarge.
//
// Отдельная ошибка, а не EOF, нужна ради клиента: EOF означал бы «данные
// битые, чини у себя», а тут чинить нечего — надо слать трек по частям.
...
// Фаззинг разбора: на любом входе Decode обязан вернуть ошибку, а не упасть.
//
// Проверяется именно отсутствие паники — результат тут не важен вовсе. Разбор
// стоит на границе с внешним миром, и паника в нём кладёт задачу целиком.
//
// В затравку положены три случая, до которых случайный перебор доходит редко:
// пустая строка, негодный base64 и годный base64 с мусором внутри. Плюс один
// настоящий закодированный трек — чтобы фаззер начинал не с нуля, а мутировал
// годный вход.
//
// Запуск: go test ./internal/codec/ -fuzz FuzzDecode
...
// Долгота 200° — такого не бывает. Проверяется и текст ошибки: по нему
// разбирающий поймёт, что виноват вход, а не мы.
//
// Пропусти мы такую точку, она уехала бы в OSRM, и он ответил бы отказом на
// весь батч — то есть одна кривая точка обесценила бы снэпы четырёх сотен
// соседей.
...
// То же для широты, и это НЕ дубль предыдущего: диапазоны у долготы и
// широты разные (±180 против ±90), и проверяются они по отдельности. Общая
// проверка по ±180 пропустила бы широту −100.
...
// Время, которого не существует: тринадцатый месяц, сорок пятое число.
//
// Формально разбирается как строка, поэтому без явной проверки такая точка
// прошла бы дальше с нулевым временем — а нулевое время это 1 января первого
// года, и вся физика на треке съехала бы на две тысячи лет.
...
// Пустой маршрут проходит круг и остаётся пустым.
//
// Случай рабочий, а не выдуманный: чистка может не оставить ни одной точки,
// и кодировщик обязан выдать годную строку, а не пустую или сломанную —
// её ещё предстоит разобрать на той стороне.
