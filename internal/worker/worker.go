...
type Pool struct {
	...
	// lastQueueOK — когда очередь последний раз ответила по-человечески:
	// выдала задачу или честно сказала, что задач нет. Ошибки не в счёт.
	//
	// По ней `/readyz` отличает живой сервис от того, который принимает
	// задачи и не разбирает ни одной. Хранится наносекундами Unix, потому
	// что atomic не умеет time.Time.
	lastQueueOK atomic.Int64
...
func (p *Pool) Start(ctx context.Context) {
	// Отсчёт «очередь молчит» начинается со старта, а не с первого удачного
	// захода. Иначе сервис, поднявшийся при лежащем Redis, не признал бы себя
	// негодным НИКОГДА: удачных заходов не было ни одного, и сравнивать не с
	// чем. А это самый обычный порядок при аварии — сначала падает Redis,
	// потом перезапускают сервис.
	p.lastQueueOK.Store(time.Now().UnixNano())
...
func (p *Pool) worker(ctx context.Context) {
	...
	for {
		...
		if err == nil || errors.Is(err, taskstore.ErrNoTask) {
			// Очередь ответила — неважно, задачей или «пусто». Отмечаем, что
			// разбор жив: пустая очередь это норма, а вот тишина из-за ошибок
		...
		if err != nil {
			...
			// Пауза перед следующей попыткой — обязательна.
			//
			// Раньше здесь был голый continue, и любая УСТОЙЧИВАЯ ошибка
			// очереди превращалась в горячий цикл: замерено на нагрузочной
			// проверке 2026-08-13 — четыре воркера дали 6.1 млн записей в лог
			// за семь минут (54 тыс. строк в секунду, 40 ГБ в час) и упёрли
			// процессор в потолок. Диск сервера такое забивает за сутки, а
			// разобрать инцидент по такому логу уже нельзя.
			//
			// Ошибка здесь либо мгновенная (Redis не отвечает, права
			// отобрали), либо не повторится вовсе — в обоих случаях бежать
			// сразу же незачем: помочь это не может, а навредить может.
			select {
			case <-time.After(dequeueErrorPause):
			case <-ctx.Done(): // остановку не ждём: гасимся сразу
				return
			}
			continue
		}
...
// queueStaleAfter — сколько молчания очереди считать поломкой.
//
// Отсчёт идёт от последнего ВНЯТНОГО ответа, включая «задач нет», а один заход
// в очередь висит blockTimeout (5 с). Значит на исправном и совершенно пустом
// сервисе отметка обновляется каждые пять секунд, и минута — это дюжина
// пропущенных подряд. Меньше брать нельзя: моргнувшая сеть выкинула бы сервис
// из балансировщика на ровном месте. var — чтобы тесты не ждали минуту.
var queueStaleAfter = time.Minute

// QueueBroken — разбор очереди мёртв: воркеры давно не получают от неё
// внятного ответа.
//
// Нужна ручке готовности. Худший отказ, найденный нагрузочной проверкой
// 2026-08-13, выглядел именно так: задачи принимаются, ни одна не считается,
// `/readyz` отвечает 200, мониторинг молчит. Проверка «жив ли процесс» такое
// не ловит по определению — процесс жив и очень занят.
//
// Пул, который ещё не запускали, сломанным не считается: воркеров нет, и
// сказать про очередь нечего.
func (p *Pool) QueueBroken() bool {
	if p.workers <= 0 {
		return false
	}
	last := p.lastQueueOK.Load()
	if last == 0 {
		return false // пул ещё не запускали
	}
	return time.Since(time.Unix(0, last)) > queueStaleAfter
}

// dequeueErrorPause — сколько ждать после неудачной попытки взять задачу.
//
// Секунда выбрана не по замеру, а по смыслу: это не «умное восстановление», а
// предохранитель от горячего цикла. Он снижает частоту повторов в десятки
// тысяч раз (было 54 тыс. попыток в секунду на четыре воркера), и при этом
// задержка в секунду ничего не стоит — задачу всё равно некому отдать, пока
// очередь не починится. var — чтобы тесты не ждали.
var dequeueErrorPause = time.Second
...
func (p *Pool) process(claim taskstore.Claim) {
	...
	// Задача уже доведена до конца — считать нечего, только расписаться.
	//
	// Номерок приходит повторно в двух случаях. Первый: расписка не прошла
	// (моргнул Redis), и уборщик вернул задачу в работу. Второй появился
	// вместе с восстановлением группы потребителей — группа живёт отдельно от
	// ленты, и, потеряв её, читать приходится С НАЧАЛА, иначе задачи, которых
	// никто не взял, пропали бы совсем. Значит воркеру снова приносят номерки,
	// посчитанные час назад.
	//
	// Пересчитывать их — не просто работа впустую (на сотне маршрутов это семь
	// минут и лишняя нагрузка на маршрутизатор): готовый ответ перезаписался бы
	// другим. Под нагрузкой дорисовка успевает по-разному, и второй ответ на
	// тот же вход отличается — замерено, 2417.9 против 2408.9 км.
	if card.Status == taskstore.StatusDone || card.Status == taskstore.StatusFailed {
		log.Info("task is already finished, skipping", "status", card.Status)
