...
const (
	...
	// SnapMinFraction — доля точек, про которые маршрутизатор обязан ответить,
	// чтобы чистке было на чём стоять.
	//
	// Вес точки без снэпа — ноль, то есть цепочке она безразлична. Поэтому при
	// нехватке снэпов ядро оставляет ровно те точки, про которые ответ пришёл,
	// и километраж падает ПРОПОРЦИОНАЛЬНО. Замер на треке в 2000 точек:
	//
	//	снэпов 100 %  → 2000 точек, 444.6 км
	//	снэпов  50 %  → 1000 точек, 222.2 км
	//	снэпов   5 %  →  100 точек,  22.0 км
	//	снэпов   0 %  →    1 точка,   0.0 км
	//
	// Ошибка кратная и ничем себя не выдаёт: ни ошибки, ни предупреждения,
	// просто вдвое меньше денег. Отдать такое молча нельзя.
	//
	// Порог высокий намеренно: на восьми настоящих треках доля ровно 100 %, до
	// единой точки. Значит недобор — неисправность маршрутизатора, а не
	// свойство данных, и терпеть его незачем.
	SnapMinFraction = 0.9
)

// ErrSnapsTooFew — маршрутизатор ответил про слишком малую долю точек.
//
// Отказ, а не деградация. Отдать НЕочищенный трек хуже, чем не отдать ничего:
...
type Report struct {
	...
	// невозможные переходы. Значит отдать её честнее, чем записать в карточку
	// `failed` — ошибка ограничена (у прототипа невыловленные переходы это 2 %
...
func (c *Core) Run(ctx context.Context, pts []geo.Point) ([]int, Report, error) {
	...
	// Предохранитель. Проверяем только когда источник снэпов ЗАДАН: пустой
	// `OSRM_URL` — осознанная настройка, а не поломка, и отказывать в ней
	// нельзя.
	if c.Snap != nil {
...
// snapFraction — доля точек, про которые маршрутизатор ответил.
func snapFraction(ok []bool) float64 {
	if len(ok) == 0 {
		return 0
	}
	have := 0
	...
	return float64(have) / float64(len(ok))
}
