...
// batchPathEps — насколько путь должен быть короче, чтобы считаться короче.
//
// Микрометр. Порог нужен не для геометрии, а против шума сложения: два
// кандидата бывают зеркальными друг другу (когда точка до пачки и точка после
// стоят в одном месте), и тогда это одни и те же отрезки, сложенные в обратном
// порядке. Сложение вещественных чисел не ассоциативно, сумма расходится в
// последнем бите — и ничья превращалась в победу того кандидата, которому
// повезло с округлением.
//
// Найдено сквозной сверкой с прототипом на `00d2e77a`: 946 переставленных
// точек против 941, дальше расходился весь трек. В прототипе те же две суммы
// совпали побитово случайно — там другая форма гаверсинуса.
//
// Величина взята с огромным запасом в обе стороны: шум сложения на пачке из
// дюжины точек порядка 1e-13 м, а любая осмысленная разница путей — это метры.
const batchPathEps = 1e-6
...
func bestOrder(pts []geo.Point, batch []int) []int {
	...
	for _, c := range candidates[1:] {
		// Побеждает только ЗАМЕТНО более короткий путь; при равных остаётся
		// первый кандидат, то есть исходный порядок. Иначе выбор зависел бы от
		// порядка перебора, и на залипшей пачке, где все пути нулевые,
		// результат гулял бы от прогона к прогону.
		if l := batchPath(pts, c, prev, next); l < bestLen-batchPathEps {
			best, bestLen = c, l
		}
	}
	return best
