...
// Треки короче трёх точек. Проверяется не только отсутствие паники, но и что
// порядок вернулся ТОЖДЕСТВЕННЫМ: перестановка отдаёт индексы, и «ничего не
// делать» здесь означает 0,1,2 — а не пустой список и не любой другой.
...
// Пачка упирается в конец трека, и точки за ней, к которой можно было бы
// пристроить порядок, нет. Случай обычный: связь пропала, буфер выгрузился
// последним, чем трек и кончился.
//
// Допуск в метр — не небрежность: пути сравниваются в метрах, и требовать
// точного равенства значило бы сторожить последний бит арифметики.
...
// Контракт всей перестановки: наружу уходят НОМЕРА, сам трек не двигается.
// Иначе стадии после ядра получили бы точки в порядке, которого не было во
// входе, а сверка с эталоном шла бы по разным данным.
