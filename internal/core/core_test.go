...
// Ядро целиком: веса, все правила, выбор цепочки, проверка по дорогам — и всё
// это до дюжины проходов. Двадцать тысяч точек с шагом полминуты — это семь
// суток записи, порядок настоящего длинного рейса.
//
// Маршрутизатор здесь подставной и отвечает мгновенно. Замер поэтому меряет
// НАШУ стоимость, без сети: на живом OSRM он же занимает секунды, и за ними
// собственная квадратичность спряталась бы незаметно.
