...
// FilterBySpeed выбрасывает точку, если от предыдущей оставленной до неё
// пришлось бы лететь быстрее MaxKmh.
//
...
//
// Замер, стоивший ей места: зацепившись за один глюк, фильтр унёс 81 честную
// точку подряд. Глюк стал последней оставленной точкой, и всё настоящее после
// него оказалось «слишком далеко слишком быстро» — пока трек случайно не
// вернулся к глюку поближе.
//
// Против спуфинга он к тому же бесполезен по построению: подделка едет
// 90–110 км/ч по настоящим дорогам и в любой порог укладывается.
