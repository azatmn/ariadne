// Package pipeline — конвейер очистки трека: упорядоченная цепочка стадий,
// каждая из которых получает точки и возвращает точки.
//
// Состав и смысл порядка описаны у New. Тяжёлая работа — выбор того, каким
// точкам верить — живёт в пакете core и занимает здесь ровно одну стадию;
// остальные стадии простые и знают только про геометрию.
//
// Разделение на стадии не косметическое: оно позволило снять четыре фильтра из
// первой версии, не трогая остальной код, и добавить дорисовку по дорогам
// одной строкой в New.
package pipeline
