// Package api — HTTP-слой асинхронного сервиса: приём задач в очередь, опрос
// результата по ключу, пробы жизни и готовности.
//
// Работы здесь нет никакой: хендлер только кладёт карточку в Redis и отдаёт
// номерок. Чистка идёт в воркере, потому что она занимает десятки секунд — на
// таком запросе HTTP-соединение отвалится по таймауту раньше ответа.
//
// Синхронная ручка (принять трек и вернуть очищенный тем же ответом) живёт
// отдельно, в internal/debugapi: она нужна для разбора и отладки, а под нагрузку
// не рассчитана.
package api
