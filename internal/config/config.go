...
//
// Читается один раз на старте, дальше Config только читают. В окружение
// вынесено далеко не всё: пороги самой чистки заданы константами в коде
// намеренно, см. комментарий у полей Config.
package config
...
// Config — все настройки сервиса. Заполняется Load, дальше только читается.
type Config struct {
	Port                 string
	GRPCPort             string
...
// Load читает окружение в Config. У каждой настройки есть умолчание, поэтому
// пустое окружение — рабочий случай: сервис поднимется на умолчаниях.
//
// Негодное значение (INT_ПОЛЕ=abc) НЕ роняет старт: в лог уходит warn, в поле
// ложится умолчание. Опечатку в переменной видно только по этой строке в логе.
//
// Ошибку Load сегодня не возвращает никогда — второе значение всегда nil.
// Место под проверки оставлено в сигнатуре заранее, самих проверок нет.
